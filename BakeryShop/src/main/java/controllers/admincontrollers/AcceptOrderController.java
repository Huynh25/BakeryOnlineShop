...
// Kiểm tra xem có thông tin nhân viên trong session hay không
        if (staffID != 0) {
            staff = staffDAO.findByID(staffID);
            System.out.println("hello + " +staffID);
        }

        orderDAO.acceptOrder(orderID, staff.getStaffID());

        request.getRequestDispatcher("/MyConfirmedOrders").forward(request, response);
    }
